...
class TrainLoop(ConfigScript):
    ...
    def unroll(self, metaconfig: MetaConfig):
        ...
        # utils for splitting params per-host
        def get_param_shapes(rng):
            print(jax.tree_util.tree_map(lambda x: type(x), model.init_weights(rng, (1, 1,)), is_leaf=lambda x: isinstance(x, (dict, FrozenDict, optax.EmptyState,))))
        ...
        # split the parameters per-host
        print(jax.tree_util.tree_map(lambda x: type(x), params, is_leaf=lambda x: isinstance(x, (dict, FrozenDict, optax.EmptyState,))))
        with Mesh(mesh_devices, ("dp", "mp")):
            rng, new_rng = jax.random.split(rng)
            host_param_shapes = jax.eval_shape(p_get_param_shapes, new_rng)
        with jax.default_device(jax.devices('cpu')[0]):
